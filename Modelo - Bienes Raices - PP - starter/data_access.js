(function () {
  function getID(array) {
    if (array.length == 0) {
      ...
      var maxIndex = array.reduce(function (prev, curr, index) {
        if (parseInt(prev.id) > parseInt(curr.id)) return parseInt(prev.id);
        else return parseInt(curr.id);
      });
  ...
  let dbObject = {
    ...
    get: function (cb, response) {
      let array;
      require("fs").readFile(__dirname + "\\data\\data.json", "utf8", function (
        err,
        data
      ) {
        if (err) {
    ...
    update: function (cb, response, nuevoObjeto) {
      ...
      require("fs").readFile(__dirname + "\\data\\data.json", "utf8", function (
        ...
        var index = array.findIndex(function (obj) {
          return (
            obj.id === nuevoObjeto.id || obj.id.toString() === nuevoObjeto.id
          );
        });
